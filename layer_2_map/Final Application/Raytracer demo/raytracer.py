...
def trace_objects(data, pixels, timestamp, camera_relative_position, camera_orientation, map_rotation,
                  classification_counter, camera_data):
    ...
    for i in camera_data:
        if i['sec'] == timestamp:
            ...
            object_image_z = i['object_image_z']
            object_image_width = i['object_image_width']

    if classification_type == 0:
        initial_orientation = 225 - map_rotation

        limit_x, limit_y = 0, 0
        object_points = []

        # align axis of map with axis of picture
        camera_relative_position = (-camera_relative_position[0], camera_relative_position[1])
        ...
        if camera_relative_position[0] <= 0:
        ...
        elif camera_relative_position[0] >= 0:
            x_transformed = (camera_relative_position[0] / resolution) * np.cos(np.deg2rad(map_rotation)) - \
                            (camera_relative_position[1] / resolution) * np.sin(-np.deg2rad(map_rotation))
        ...
        camera_rotation = quaternion_to_euler(camera_orientation) + map_rotation + initial_orientation
        ...
        if classification == "static":
            ...
            object_points = shoot_rays(camera_position, pixels, start_right, stop_right, start_up, stop_up, start_left,
                                       stop_left, start_down, stop_down, object_points,
                                       ray_endpoint_start_x, ray_endpoint_start_y, ray_endpoint_stop_x,
                                       ray_endpoint_stop_y)
            ...
            ray_endpoint_start_x_2, ray_endpoint_start_y_2 = calculate_viewpoints(camera_rotation,
                                                                                  (ray_endpoint_start_x,
    ...
    data.save('data/map/layer 2/mapLayer2_{}.png'.format(classification_counter))
...
def get_data():
    files = os.listdir('data/positional data')
    classification = []
    object_image_x = []
    object_image_z = []
    object_image_width = []
    ...
    for i in sorted(files):
        print(i)
        with open('data/positional data/' + i) as json_file:
            ...
            data2.append({"sec": data['sec'], "position": {"x": data['pos']['x'], "y": data['pos']['y']}, "orientation":
                {"x": data['orientation']['x'], "y": data['orientation']['y'], "z": data['orientation']['z'],
                 "w": data['orientation']['w']}})
            # TODO: uncomment
        os.remove('data/positional data/' + i)
...
def get_map(counter):
    ...
    data = Image.open('data/map/layer 1/mapLayer1_0.bmp').convert('RGB', colors=256)
    pixels = data.load()
    opencv_im = cv2.imread('data/map/layer 1/mapLayer1_0.bmp')

    return data, pixels, opencv_im
    # else:
...
def generate_camera_data():
    camera_data = []
    # file 104
    camera_data.append({"sec": 2589, "classification": "static", "object_image_x": 305, "object_image_z": 2000,
                        "object_image_width": 235})
    # file 43
    camera_data.append({"sec": 2467, "classification": "static", "object_image_x": 652, "object_image_z": 2000,
                        "object_image_width": 70})
    # file 60
    camera_data.append({"sec": 2547, "classification": "dynamic", "object_image_x": 270, "object_image_z": 2000,
                        "object_image_width": 200})
    return camera_data
...
def main():
    ...
    while True:
        data, pixels, opencv_im = get_map(classification_counter)

        map_rotation = calculate_map_rotation(opencv_im)

        data2 = get_data()

        data2 = sorted(data2, key=lambda k: k['sec'])

        print(data2)

        for i in data2:
            trace_objects(data, pixels, i['sec'], (i['position']['x'], i['position']['y']), (i['orientation']['x'],
                                                                                   i['orientation']['y'],
                                                                                   i['orientation']['z'],
                                                                                   i['orientation']['w']), map_rotation,
                          classification_counter, camera_data)
