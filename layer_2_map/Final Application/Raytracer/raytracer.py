...
def trace_objects(data, pixels, classification, object_image_x, object_image_z, object_image_width,
    ...
    if camera_relative_position[0] <= 0:
        x_transformed = (camera_relative_position[0]) * np.cos(np.deg2rad(-map_rotation)) - \
                        (camera_relative_position[1]) * np.sin(np.deg2rad(-map_rotation))

        y_transformed = (camera_relative_position[0]) * np.sin(np.deg2rad(-map_rotation)) + \
                        (camera_relative_position[1]) * np.cos(np.deg2rad(-map_rotation))
    ...
    if classification == "static":
        ...
        # shoot rays
        dynamic_object_points = shoot_rays_2(ray_endpoint_start_x, ray_endpoint_start_y, ray_endpoint_stop_x,
                                             ray_endpoint_stop_y,
                                             ray_endpoint_start_x_2, ray_endpoint_start_y_2, ray_endpoint_stop_x_2,
                                             ray_endpoint_stop_y_2,
                                             pixels)
...
def main():
    ...
    while True:
        ...
        for i in range(len(classification)):
            dynamic_object_points = trace_objects(data, pixels, classification[i], object_image_x[i], object_image_z[i],
                                                  object_image_width[i],
                                                  camera_relative_position[i], camera_orientation[i], map_rotation)

            if dynamic_object_points is not None:
                dynamic_objects.append({"time": time.time(), "pixels": dynamic_object_points})

            for j in dynamic_object_points:
                if j['time'] - time.time() < 30:
                    for x in j['pixels']:
                        visualize_ray_2(x, (0, 0, 0), pixels)
